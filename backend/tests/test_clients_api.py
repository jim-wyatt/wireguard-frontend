...
def test_management_reads_require_authentication(unauthenticated_client, db_session):
    ...
    list_response = unauthenticated_client.get("/api/peers")
    assert list_response.status_code == 401

    detail_response = unauthenticated_client.get(f"/api/peers/{db_client.id}")
...
def test_dashboard_reads_are_public(unauthenticated_client, monkeypatch):
    ...
    stats = unauthenticated_client.get("/api/peers/stats")
    assert stats.status_code == 200

    connected = unauthenticated_client.get("/api/peers/connected")
...
def test_create_client_and_reject_duplicate_email(client, monkeypatch):
    ...
    response = client.post("/api/peers", json=payload)
    ...
    duplicate = client.post("/api/peers", json=payload)
...
def test_stats_and_connected_clients(client, db_session, monkeypatch):
    ...
    stats = client.get("/api/peers/stats")
    assert stats.status_code == 200
    ...
    connected = client.get("/api/peers/connected")
...
def test_toggle_and_delete_client(client, db_session, monkeypatch):
    ...
    deactivate = client.patch(f"/api/peers/{db_client.id}/toggle")
    assert deactivate.status_code == 200
    assert deactivate.json()["is_active"] is False
    assert toggled["removed"] == 1

    activate = client.patch(f"/api/peers/{db_client.id}/toggle")
    assert activate.status_code == 200
    assert activate.json()["is_active"] is True
    assert toggled["added"] == 1

    deleted = client.delete(f"/api/peers/{db_client.id}")
